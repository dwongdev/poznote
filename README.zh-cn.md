...
Poznote 支持多种身份验证方式，包括本地账户和外部身份提供商。与 REST API 通信的应用和扩展使用**应用密码**，这是一种独立的凭据，下文会详细介绍。
...
8. 完全无法执行 OIDC 流程的客户端（浏览器扩展、移动应用、脚本）改用[应用密码](#app-passwords)，每个用户可在自己的设置中创建。
...
<details>
<summary><strong>应用密码（用于应用、扩展和脚本）</strong></summary>
<br>

应用无法像浏览器那样通过身份提供商登录。**应用密码**是您为某个客户端单独创建的凭据，可以随时撤销，因此您永远不必交出账户密码。

在 **设置 > 应用密码** 中创建：为其命名（使用它的客户端），可选设置过期时间，然后复制生成的密钥。密钥只显示一次，之后不会再显示。

然后在客户端中，在要求输入密码的地方填写**您平常的用户名**和**应用密码**即可。其他一切不变：它以普通的 HTTP Basic Auth 方式传输，因此所有现有客户端都可以直接使用。
...
|  | |
|---|---|
| ✅ 读写**其所属资料**的笔记、文件夹、标签和附件 | ❌ 打开网页界面：会在登录表单处被拒绝 |
| ✅ 在仅 SSO 的实例上使用，包括启用了*为 API 禁用 HTTP Basic Auth* 的情况 | ❌ 访问任何 `/api/v1/admin/*` 端点，即使账户是管理员 |
| ✅ 设置过期日期，并可随时撤销 | ❌ 修改您的密码、编辑或删除您的账户，或创建更多应用密码 |
| ✅ 省略 `X-User-ID` 请求头：它绑定到创建它的资料 | ❌ 操作其他资料，即使是管理员也不行 |

由于这些限制，泄露的应用密码只会暴露一个账户的笔记，不会波及更多内容，而且只需撤销该条目即可堵住漏洞。

**设置 > 应用密码** 中的列表会为每个应用密码显示密钥的前几个字符（便于区分）、创建时间和最后使用时间，因此很容易发现并删除闲置的凭据。每个账户最多可以拥有 25 个。

> **在仅 SSO 的实例上**，应用密码是 API 通过 Basic Auth 接受的唯一凭据。由 OIDC 自动创建的账户完全没有账户密码，因此这些账户的所有者正是通过这种方式连接浏览器扩展、手机或脚本。

完整参考（包括管理应用密码的端点）：[REST API 文档](docs/API-REST.md#authentication)。
...
*   **工作流程：** “今日日记”按钮会打开当天的笔记（如不存在则创建），标题为当前日期，并自动存放在 `Diary/YYYY/MM` 文件夹结构中。
...
Poznote 采用多用户架构，每个资料都有相互隔离的数据空间，同时仍允许在同一实例上进行受控的协作。

- **数据隔离**：每个资料都有自己的笔记、工作区、标签、文件夹、附件和用户设置。
- **按资料进行身份验证**：用户使用自己的用户名或电子邮件地址和密码登录。在界面中修改密码之前，使用内置的默认密码（管理员为 `admin`，普通用户为 `user`）。
- **用户管理**：管理员可以在 **设置 > 管理工具 > 用户管理** 中创建、停用和管理资料。
- **委派账户访问**：管理员可以授权一个用户访问另一个用户的账户。当用户可以打开多个账户时，Poznote 会在登录后询问要使用哪个账户，并清楚地标明会话何时正**以其他用户身份**操作。
- **所有者/管理员保护**：打开其他用户的账户并不会转移所有权。修改密码、备份/还原、Git 同步配置以及全局管理设置等敏感操作，仍然仅限相应的所有者或管理员执行。
- **共享**：笔记、文件夹乃至整个工作区都可以与同一实例上的其他用户共享，也可以通过专用链接公开共享。共享默认为只读，也可以设为可编辑：笔记可编辑文本，文件夹可编辑其中的笔记，任务列表则有三个级别（只读、仅勾选或取消勾选、完全编辑）。
- **单编辑者锁定**：当多个用户可以访问同一条笔记时，Poznote 同一时间只允许一位编辑者。其他用户仍可以只读模式打开笔记，查看当前是谁持有锁，并在锁被释放或过期后重新打开笔记以接管编辑。
- **租户隔离（SaaS 模式）**：管理员可以为非管理员用户屏蔽特定功能，例如发现实例中的其他账户并与之共享，或注册个人 Webhook。管理员永远不受影响。如果是家庭或团队实例，全部保持不勾选即可。
...
Poznote 会记录在实例上执行的敏感操作的历史，让管理员可以了解发生了什么、何时发生以及由谁执行。该功能位于 **设置 > 管理工具 > 活动日志**，仅限管理员使用。

每条记录包含日期和时间、相关账户、操作以及简短摘要，例如被删除工作区的名称或被删除笔记的数量。将鼠标悬停在页面顶部的帮助图标上，可以查看所记录操作的完整列表，涵盖：

- **会话**：登录和注销。
- **账户**：资料变更（用户名、电子邮件、姓名）、配额变更、启用和停用、授予或撤销管理员角色、删除账户，以及授予或撤销委派账户访问。
- **工作区**：创建、删除、共享和取消共享。
- **数据**：创建备份和还原、清空回收站以及永久删除笔记。
- **应用密码**：创建和撤销。

日常活动被有意排除在外：编写笔记或将笔记移到回收站不会被记录，每次请求都进行身份验证的 API 调用也不会被记录，否则日志就会变成流量记录。

日志只记录某个操作发生过，而不记录它涉及的数据。**笔记内容从不写入日志**，标签、文件夹和附件也不会。删除记录只通过笔记的标题和所在工作区来标识笔记，以便识别该事件，仅此而已。

> **任何密码都不会以任何形式写入日志。** 在涉及密码的场景下（例如受保护的共享工作区），只记录是否设置了密码。

记录默认保留 90 天。保留期可以改为 30、90 或 365 天，也可以设为无限期，还可以在同一页面中手动清空日志。

## Webhook

Poznote 可以在实例上发生某些事件时通知外部服务，方法是向您注册的端点发送出站 Webhook（带 JSON 负载的 HTTP POST 请求）。这样就可以轻松地将 Poznote 接入 n8n、Zapier 或您自己的脚本等自动化工具。Poznote 只负责发出 Webhook：接收端点如何处理它们（发送电子邮件、触发工作流等）由您决定。

Webhook 分为两个级别：

- **管理员 Webhook**（**设置 > 管理工具 > 管理员 Webhook**，仅限管理员）：实例事件，例如 `user.created`、`user.updated`、`user.activated`、`user.deactivated`、`user.deleted`、`settings.language_changed`、`signup.cap_reached`、`quota.notes_reached` 和 `quota.storage_reached`。
- **用户 Webhook**（**设置 > 用户 Webhook**）：每个账户都可以为与自己内容相关的事件注册自己的端点：`note.created`、`note.shared` 以及提醒事件。这些事件只会投递到产生它们的账户所注册的端点，绝不会投递给其他用户。

当 Webhook 设置了密钥时，投递的 JSON POST 请求会使用 HMAC-SHA256 签名（与 GitHub Webhook 的方案相同）。笔记内容从不发送，负载只包含元数据；提醒事件有三种变体，由您决定有多少数据离开实例。

完整参考涵盖所有事件、确切的负载字段（`data.user`、`data.note` 等）、带代码示例的签名验证、投递保证，以及用于生成笔记直接链接的实例 URL 配置，请参阅 **[Webhook 文档](docs/WEBHOOKS.zh-cn.md)**。
...
Poznote 内置 AI 聊天功能，可以连接任何兼容 OpenAI 的服务器、本地的 [Ollama](https://ollama.com) 或 [LM Studio](https://lmstudio.ai) 实例，或者 [Anthropic (Claude)](https://www.anthropic.com)、OpenAI 等云服务提供商。配置完成后，笔记页面和仪表盘的左侧图标栏中会出现 **AI 助手** 按钮，点击即可就地打开聊天面板。

该助手是全局的，工作方式类似 MCP：它拥有**搜索和阅读您笔记**的工具，并会主动使用这些工具来回答问题，例如“我的笔记里关于 X 是怎么说的？”、跨笔记的总结，或者找到那条您只记得一半的笔记。当您明确提出要求时，它还可以**创建、重命名和改写笔记**，**整理笔记**（标签、文件夹、收藏、提醒、任务和复选框），以及**将笔记和文件夹移到回收站**（绝不会永久删除）。回答以流式方式输出，并渲染为 Markdown。

该助手**仅作用于当前工作区**：它只能查看、搜索和编辑您打开聊天时所在工作区的笔记，新建的笔记也会创建在该工作区中。如需询问其他工作区的内容，请先切换到该工作区。

要启用它，请前往 **设置 → 管理工具 → AI 助手**（仅限管理员），选择一个提供商，然后使用 **检查访问并列出模型** 验证服务器，并从它提供的模型中选择一个。该配置适用于整个实例：管理员启用后，每个用户资料都可以使用聊天功能。

管理员还可以允许使用个人 API 密钥。这样每个用户在自己的设置中都会有一个 **我的 AI 助手** 卡片，可以让聊天使用自己的服务器、提供商和密钥，而不是实例的配置。

完整的配置指南涵盖提供商、模型选择，以及如何从 Poznote 容器连接本地 Ollama/LM Studio 服务器（找到正确的 URL、`OLLAMA_HOST`、Docker 网络），请参阅 [AI 助手文档](docs/AI-ASSISTANT.zh-cn.md)。

AI 服务器由 Poznote 服务器调用，绝不会从您的浏览器调用。使用本地 Ollama 实例时，您的笔记和对话永远不会离开您的机器。如果想让外部 AI 助手（VS Code Copilot、Claude CLI 等）来管理您的笔记，请参阅下文的 [MCP 服务器](#mcp-服务器)。
...
Poznote 包含一个 Model Context Protocol (MCP) 服务器，让 GitHub Copilot 等 AI 助手可以通过自然语言操作您的笔记。例如：
...
有关设置和使用说明，请参阅 [MCP 服务器文档](docs/MCP-SERVER.zh-cn.md)。

MCP 服务器使用默认设置（端口 `8045`，关闭调试）。如需覆盖：

```bash
POZNOTE_MCP_PORT=9000 POZNOTE_DEBUG=true docker compose up -d --force-recreate mcp-server
```

这些是容器/运行时的覆盖参数，而不是 Poznote 界面中的设置。您可以像上面那样在命令行中直接传入，也可以在重新创建 `mcp-server` 容器之前将它们写入 `.env`。

MCP 服务器对 `POZNOTE_DEBUG` 只识别完全小写的 `true` 和 `false`，其他任何值都会回退为 `false`（Web 服务器的容忍度更高，还接受 `1`、`on` 或 `yes`）。修改设置后，请重新创建容器；简单的重启不会重新加载环境变量。

**安全性：** MCP 端口只发布在 `127.0.0.1` 上，因此默认情况下，您机器之外的任何设备都无法访问它。如果您将其进一步暴露（反向代理、局域网或非 Docker 安装），请在 `.env` 中设置 `POZNOTE_MCP_AUTH_TOKEN`，服务器将要求每个客户端提供 `Authorization: Bearer <token>` 请求头。在 Docker 之外运行时，`poznote-mcp serve` 默认绑定到 `127.0.0.1`。详情请参阅 MCP 文档中的[安全部分](docs/MCP-SERVER.zh-cn.md#安全)。
...
这里也可以使用您的账户密码，但对于扩展来说，应用密码是更合适的凭据：它只能访问 API，永远无法进入网页界面或您的账户设置，仅限于您自己的资料，而且在 Poznote 中撤销它即可切断扩展的访问，不会影响其他任何东西。完整的限制列表请参阅[应用密码](#app-passwords)。
